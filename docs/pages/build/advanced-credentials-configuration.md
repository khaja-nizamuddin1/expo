...
Once you have the Distribution Certificate and Provisioning Profile on your computer, you should move it to the appropriate directory. We recommend you keep them in the `ios/certs` directory and we assume that in the rest of this document they are named `dist.p12` and `profile.mobileprovision`, respectively. **Remember to git ignore all files in the directory!** If you've placed the credentials in the directory suggested by us, you can ignore those files by adding the following line to `.gitignore`:
